...
def admin_headers(service_role: str) -> dict:
    return {
        "apikey": service_role,
        "Authorization": f"Bearer {service_role}",
        "Content-Type": "application/json",
...
def patch_user_app_metadata(service_role: str, user_id: str, metadata: dict) -> None:
    headers = admin_headers(service_role)
    user = request_json("GET", f"{supabase_url}/auth/v1/admin/users/{user_id}", headers)
    existing = user.get("app_metadata") or {}
...
def jwt_claim(token: str, claim: str) -> str | None:
...
def main() -> None:
    ...
    if not refresh_only:
        ...
        config_push = subprocess.run(
            ["supabase", "config", "push", "--yes"],
            cwd=root,
            capture_output=True,
            text=True,
        )
        print(config_push.stdout.strip() or "supabase config push: ok")
        if config_push.returncode != 0:
            print(config_push.stderr, file=sys.stderr)
            config_push.check_returncode()
    ...
    patch_user_app_metadata(service_role, mac_account_id, {"mac_agent_id": mac_agent_id})
    patch_user_app_metadata(service_role, ios_account_id, {"remote_device_id": rls_device_a_id})
    ...
    mac_claim = jwt_claim(mac_session["access_token"], "mac_agent_id")
    if mac_claim != mac_agent_id:
        raise SystemExit(
            f"Mac JWT missing mac_agent_id claim (got {mac_claim!r}); "
            "confirm custom_access_token_hook is enabled on the linked project"
        )
