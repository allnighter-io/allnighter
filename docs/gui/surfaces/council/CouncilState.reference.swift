...
/// The three top-level screens, plus the synthesizer sub-state of `.run`.
enum CouncilView: Equatable { case compose, run, plan }
enum SynthPhase: Equatable { case waiting, synthesizing, ready }

// MARK: - The founder's six-worker panel (MVP)

enum CouncilData {
    static let workers: [Worker] = [
        Worker(id: "opus",     name: "Opus 4.8",     model: "via claude-code", brandSlug: "anthropic",    glyphHex: 0xFFA630, isSynthesizer: true),
        Worker(id: "gpt",      name: "ChatGPT 5.5",  model: "via codex-cli",   brandSlug: nil,            glyphHex: nil),   // SF Symbol "terminal"
        Worker(id: "sonnet",   name: "Sonnet 4.6",   model: "via claude-code", brandSlug: "anthropic",    glyphHex: 0xAEB5C9),
        Worker(id: "composer", name: "Composer 2.5", model: "via cursor",      brandSlug: nil,            glyphHex: nil),   // SF Symbol "square"
        Worker(id: "gemini",   name: "Gemini Flash", model: "via gemini-cli",  brandSlug: "googlegemini", glyphHex: 0xE1E5F0),
        Worker(id: "grok",     name: "Grok Build",   model: "via grok-cli",    brandSlug: "x",            glyphHex: 0xE1E5F0),
    ]
}
...
final class CouncilModel {
    ...
    var selectedWorkers: [Worker] { CouncilData.workers.filter { selected.contains($0.id) } }
    ...
    // When the slowest finishes: synth waiting → synthesizing (+450ms) → ready (+2.7s).
    ...
    func startRun() {
        ...
        //       When all have terminated, set synth = .synthesizing, run Opus on the
...
/*
 ...
 ──────────────────────────────────────────────────────────────────────
 from .compose
   Run council (canRun)         → .run, synth .waiting, all seats .queued
 within .run
   per worker (stagger ~170ms)  → .running           (live dot pulses)
   per worker (on CLI exit)     → .done | .failed | .timedOut (+ meta)
   all workers terminated       → synth .synthesizing (live mark blinks)
   synthesis complete           → synth .ready        ("View master plan")
   Stop                         → .compose            (clear timers/tasks)
   View master plan (.ready)    → .plan
 within .plan
   New run                      → .compose (reset)
   Tabs                         → segmented "Master plan" / "Member answers"
 ──────────────────────────────────────────────────────────────────────
