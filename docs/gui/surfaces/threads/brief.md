...
They live alongside the existing Council UI; Home does not flip to threads until
PWT-S08.
...
- Ask panel / Turn into work order / Dispatch / Continue from this → escalation
...
- council/build → compact expandable rich turn reusing existing council/
  master-plan/dispatch/return-review cards, referenced by `runId`/`stageId`.
- system → muted note; migration ("Imported council run — no prior chat")
...
| Council/build card | `turn.runId` → `CouncilRun` | RunStore | populated | (existing run cards) |
...
- Hide the plumbing: copy says worker / panel / council / master plan, never
  subprocess / worktree / branch.
