...
| Doc | Status | Purpose |
| --- | --- | --- |
| [`Run_Lifecycle_Reliability.md`](Run_Lifecycle_Reliability.md) | **FINAL; S00+S01+S02 DELIVERED 2026-07-19 — S03 next; blocks IR-S02 + Onboarding V1** | S00 evidence (incident packet, spawn-site matrix, fake CLI + gated-red tests) + S01a/b/c landed: RunLifecycle/RunPhase wire converged, one-worker `fanning_out` dead (red signature (a) green), durable-before-waits acceptance, JOURNAL_CORRUPT/RUN_JOURNAL_UNAVAILABLE typed, clock defaults named. Terminal-lie signature stays red for S04. Debt: `check.sh` blocked by pre-existing spawn-policy gate (bare SubprocessCommandRunner). |
