...
| [`OpenCode_Go_Capacity.md`](../archive/phases/OpenCode_Go_Capacity.md) | **KILLED (2026-08-12)** — same ruling. Chrome cookie → AES-GCM store → HTTPS fetch → SSR tokenizer, built to feed the capacity strip. Never started; no code lost. | None — do not build credential storage for a dashboard |
| [`Bailian_Token_Plan_Capacity.md`](../archive/phases/Bailian_Token_Plan_Capacity.md) | **KILLED (2026-08-12)** — same ruling. Vendor-specific quota reader for the same unread dashboard. Existing dogfood spike stays as-is, not extended. | None |
...
| Work | Read first |
| --- | --- |
| Capacity strip, `alln capacity`, warm pool, stale % | Bench tally / never-scanned host / a seat wrongly shown missing | Archived [`First_CLI_Detection_And_Setup_Code_Red.md`](../archive/phases/First_CLI_Detection_And_Setup_Code_Red.md); law in [`setup/01_CLI_Detection_Auth_And_Bench.md`](setup/01_CLI_Detection_Auth_And_Bench.md); code SSOT `BenchTallyProjector`, `ProbeRecordMerge` |
