...
| [`Unified_Run_Model.md`](Unified_Run_Model.md) | **Decision — replacement root model** | A run = message + optional preset + worker, in the repo root. Default chat = the Default Team (raw passthrough). Answer teams (parallel, read-only) vs execution teams (one worker, mutating, write-locked). Replaces the deleted Project Manager / work-order / propose→dispatch→verify loop. |
