...
| [`Agent_Visible_Queuing.md`](Agent_Visible_Queuing.md) | **OPEN — incident-driven** | Agent status honesty: FIFO queue truth at dispatch (not silent `.queued`) + **running≠advancing** (`progressStale`/`silenceStatus` must scream, not hide under `HB_AGE`). One mutating worker per repo root. AVQ-S00–S07. Origin: 2026-07-30 triple-dispatch + Grok stall while polling `running`. |
