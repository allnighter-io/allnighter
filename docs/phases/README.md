...
| [`OpenCode_Headless_Completion_And_Session_Scoping.md`](OpenCode_Headless_Completion_And_Session_Scoping.md) | **SPEC — opened 2026-08-07; not built** | OpenCode/alln silent failures: unscoped global `session.idle` aborts Pro mid-turn; prompt filed as answer; `external_directory` hang; commit-only `repoDelta`. Fix in AgentOS SSE client + Allnighter outcome gates (S122.0–S122.5). Do not demote DeepSeek V4 Pro. |
