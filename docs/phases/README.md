...
| [`Team_Catalog.md`](Team_Catalog.md) | **Backend BUILT-COMPLETE** (S00-S05, historical); forward catalog work owned by `Team_And_Skill_Catalogs.md` | Built-in Code/Design/Copy specialist team catalog substrate for Send to team. Shipped + stable (`TeamCatalog.swift`); this is the completed M1 substrate spec — candidate to archive once readers rely on the code + the catalog-editing doc. |
