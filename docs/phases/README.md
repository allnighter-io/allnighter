...
See [`Test_Infrastructure_Upgrade.md`](Test_Infrastructure_Upgrade.md) (FINAL rev 3 —
100%-AI-run, zero human backstop: S00–S03 all core, PATH shim + token makes the
wrapper the only working path on every agent host; GitHub Actions CI deferred
separately).
...
| Doc | Status | Purpose |
| --- | --- | --- |
| [`Test_Infrastructure_Upgrade.md`](Test_Infrastructure_Upgrade.md) | **FINAL rev 3 — ready to implement** | 100%-AI-run repo, no human backstop: lock + kill + timeout wrapper, PATH shim + single-use token (raw `swift test`/`xcodebuild test` don't work outside the wrapper, on any agent host), `check-fast`, serialize hang-prone suites. No Kanso-style policy engine/audit log in core. TIU-S00–S03 all core, order S00→S03→S01→S02 (~2.5–3 days); GitHub Actions CI deferred separately. |
...
| [`Loop_Verb_Cutover.md`](Loop_Verb_Cutover.md) | **v7 LOCKED 2026-07-30 — S01/S03 implementing** | The old relay/pilot dispatch verbs retire in favor of **`alln loop start "<what you want done>"`** — the brief is the ONLY required input; `--spec`, `--pm`, `--dev` all optional (seats default by tier). Plus `list\|status\|stop\|resume\|wait\|step\|pm`. Grammar derived from the founder's own sentences, not taxonomy (§2): he varies **casting**, never posture — so there is no mode word, the kind slot ships **empty**, and `--spec` is a shortcut not the shape. Laws: kind (when one exists) names what *done* means; the PM chair is an occupant; operations are defined against the state machine, never the chair. Kills the old relay/pilot verb pair, the retired PM-chair enum, the retired flag trio, and both legacy `adopt`s. Contract 7.0.0 / binary 0.11.0. Reviewed: Grok 4.5 doc review, Fable 5 cold redesign, Sonnet 5 readiness. **Supersedes `Product_Vocabulary.md` L27/L29/L51-53.** Mac: Delivery Loop. Sheet → §8; symbol sweep → S09; **archive → S10**. |
...
| Work | Read first |
| --- | --- |
| Plan-time quota routing, capacity in menu/bootstrap, loop dies on session cap instead of waking at reset, cross-vendor seat arbitrage | [`Quota_Aware_Bench_Continuity.md`](Quota_Aware_Bench_Continuity.md) — code SSOT `CapacityDisplayAcquisition`, `MenuCatalog`, `VendorBackoffReconciler`, `RelayCoordinator.dispatchDevTurn`, `VendorSubstitutionPolicy` |
