...
| [`Team_Run_Load_Performance.md`](Team_Run_Load_Performance.md) | Investigation + recommendation only — no runtime fixes yet | Dogfood diagnosis for 5–10s app lock when opening the first large Team run: actual files checked, JSON verdict recorded, root issue framed as synchronous full-run decode/Markdown render plus thread-as-reader navigation. Recommends summary sidecars, async cached run loading, lazy Floor rendering, and performance gates targeting <50ms first response. |
...
| [`Team_And_Skill_Catalogs.md`](Team_And_Skill_Catalogs.md) | **S00–S04 BUILT** (catalogs/IDs/persistence/CLI/resolver); S05 (Mac Settings lane-first nav) in progress | Cleanup-first lane catalog feature: `TeamCatalog` + `SkillCatalog`, `TeamID` + `SkillID`, built-in and custom teams/skills in one catalog model, lane-first Settings, no Store vocabulary, no migration, no skill versioning. |
