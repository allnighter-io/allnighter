...
| [`Capacity_Serve_Refresh_Polish.md`](Capacity_Serve_Refresh_Polish.md) | **OPEN — packet only; code unauthorized** | Post–PF-S03 residuals from OpenCode audits (Pro `A655F640`, Qwen `5DD71ADC`): probe scope on serve shutdown, freshness margin + jitter, partial-failure retry ledger, async refresh + backoff. Preserves no-lock / §0.2. |
