...
| [`Menu_Envelope_Compression.md`](Menu_Envelope_Compression.md) | **OPEN — design recommendation** | QABC-S00 prerequisite: live menu is 35 KB / 32 KiB cap; tests gate built-in fixture only. **Recommendation:** keep default `alln menu --json` as plain readable JSON (no gzip stdout); gate live projection; revisit cap before encoding tricks. Usability > wire cleverness. |
