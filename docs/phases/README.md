...
| Doc | Status | Purpose |
| --- | --- | --- |
| [`Agent_Onboarding.md`](Agent_Onboarding.md) | **Specced v2** (five-seat panel-hardened 2026-07-16) | From findable to suggested: the app teaches every CLI session about alln — trigger-line snippet, one-click global install, recipe gallery. V1 = 3 slices; per-project offer + graduation nudge parked. |
...
| [`MCP_Run_Factory_Team_Lab.md`](MCP_Run_Factory_Team_Lab.md) | Draft feature packet — **⚠ STALE PREMISE** (MCP retired 2026-07-16, `MCP_Retirement.md`; lab harness must be re-based CLI-native `alln` before implementation) | MCP-only run factory for making default Teams excellent: benchmark suites, full MCP transcripts, per-worker/writer scoring, run-contract scoring, artifact completeness checks, and stop-the-line fixes for MCP/run bugs found while calibrating Teams. |
...
| [`Team_Lab_Composition_And_Seat_Economics.md`](Team_Lab_Composition_And_Seat_Economics.md) | **Active implementation spec** (2026-06-23) | Post–Slice 1 macro loop: seat economics (benefit − cost), VNRC, forward selection, asymmetric ADD/REMOVE gates, necessity suite, named teams (Lite / Bug Hunt / Forensics), LAB-C00–C08. Mentor review incorporated. Variant naming superseded by `Team_Depth_Naming.md`. |
| [`Team_Depth_Naming.md`](Team_Depth_Naming.md) | **DECIDED — convention SSOT** (founder, 2026-07-16) | Family = the job, named by what it does (Spec Review, Bug Hunt; "Pressure Test" stays a marketing/loop name only). Depth = universal Min / (bare name) / Max; bare name is the default send; picker renders Min / Default / Max. Min/Max are Team-Lab-defined (necessity suite / forward selection), no numbers in names, rename map for the bug family included. Routing law: default send always → bare team, never Min. "Pressure Test" retired everywhere (marketing included). **Rename slice runs FIRST, before Team Lab** (cutover is DONE; lab artifacts must be born with final IDs). |
...
| MCP-only run factory, default Team benchmarking/calibration, per-worker scoring, run-contract bugs found through MCP | `MCP_Run_Factory_Team_Lab.md` + `Agent_First_MCP_And_Messaging_Workflows.md` + `CLI_Implementation_Contract.md` + `Team_Run_Floor.md` |
| Team lab seat economics, roster ablation, named team variants, necessity suite, macro vs micro loop | `Team_Lab_Composition_And_Seat_Economics.md` + `MCP_Run_Factory_Team_Lab.md` + `Team_Lab_Slice_1_Full_Package.md` |
