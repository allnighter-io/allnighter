...
| [`Loop_Verb_Cutover.md`](Loop_Verb_Cutover.md) | **Draft v3 — founder approved 2026-07-30, reviewed, naming closed, not started** | `alln pair relay\|pilot` → `alln loop delegate\|pilot`. `loop` is a sibling verb of `run` on containment (a loop owns many `runId`s); Menu_Not_Router permits it but requires declared effects + a free `--dry-run` twin (LVC-S08). Hard cutover, no aliases; contract 7.0.0 / binary 0.11.0; frees `alln pair` for device pairing; Mac offers Delegation Loop only. **Supersedes `Product_Vocabulary.md` L27/L51-53.** Naming law: **the type slot names what the loop DOES; attendedness is orthogonal and must never be spent there** (`autopilot` rejected on that ground; `pilot` is grandfathered debt). Open in S00: two opposite adopt directions, status collapse, `--relay` flag ruling. Sheet redesign deferred to §8. |
