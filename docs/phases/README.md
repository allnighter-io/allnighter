...
| --- | --- | --- |
| [`Run_Readout_Truth.md`](Run_Readout_Truth.md) | **0 · Ready-first fix — v3, Ready for named slices (S01+S04)** | `errors: []` is a **hardcoded literal** (`TeamRunJSONMapper:238`) — no run has ever reported a failure reason at top level. A completed Ready run read as dead; a failed run gave no reason while it sat in four other keys. Reviews: Grok `411F2E17` Not ready + DeepSeek `AC2E53DC` Ready, adjudicated §0. Net-negative fields, no new command/flag/daemon. |
