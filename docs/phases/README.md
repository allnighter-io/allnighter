...
| [`Agent_Visible_Queuing.md`](Agent_Visible_Queuing.md) | **OPEN — incident-driven** | Agents dispatching mutating runs must see FIFO queue truth immediately — not silent `.queued`. One mutating worker per repo root (not driver-specific). AVQ-S00–S04: dispatch ticket, `floor status`, bootstrap teaching, reconcile→release lock, peer notify. Origin: 2026-07-30 triple-dispatch surprise (Grok holder; Cursor zombie red herring). |
