...
| --- | --- | --- |
| [`Local_Runtime_Surface.md`](Local_Runtime_Surface.md) | **READY FOR IMPLEMENTATION — founder-ruled 2026-08-14** | Packet 1 shipped a working local seat pipeline and never wired it to a command or a screen. Verified: `OllamaLocalModelDiscoveryProvider.discover()`, `ModelDiscoveryRegistry.provider(for:)`, and `OllamaLocalSeatEnablePolicy.assessExplicitEnable()` all have **zero production call sites**; `alln models` shows 43 seats and 0 local; the Mac app has no Ollama concept at all; `opencode-local` serves a frozen tag list missing a model pulled the same day. Reverses no law — `defaultEnabled = false` stands; the defect is that *off* means *invisible*. Pressure-tested twice by Grok 4.6; §0.3/§0.5 carry the verified code state and the four wrong turns not to repeat. Start at **LR-S00**, a catalog-path audit — Teams declining an unpinned local seat is packet 1 ruling 11 working, **not** a blocker. |
...
| Local models missing from the app or `alln models`; a newly pulled Ollama tag not appearing; which body runs a local model | [`Local_Runtime_Surface.md`](Local_Runtime_Surface.md) — §0.3 is verified state, not doc-read |
