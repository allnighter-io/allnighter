...
| [`Message_Image_Rendering.md`](Message_Image_Rendering.md) | **Partially built** — engine/MCP send landed; GUI + read-path enrichment remain | Umbrella spec: images in thread messages (single-lane worker replies, user attachments, Design fan-out tiles). MCP-first (`thread_get` attachment resolve, design run paths) then Mac timeline/board/Floor rendering. |
