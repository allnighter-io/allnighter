...
| [`Completion_Delivery.md`](Completion_Delivery.md) | **OPEN — incident-driven (2026-07-30)** | **Agent completion notification** (CLI→CLI): when dev/run finishes, originator gets terminal JSON via existing waiters (`delivery.command`, `--wait-for`), not Mac banners. CD-S01 relay parked truth first. Origin: PM said "still running" after dev done. |
