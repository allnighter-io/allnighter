...
| [`One_Paste_Cold_Start.md`](One_Paste_Cold_Start.md) | **OPEN — S05 CLI + DMG live** | Current floor **1.1.3**. `curl -fsSL https://get.allnighter.io \| sh` + `/Allnighter.dmg`. Repeatable ship: `docs/operations/Public_Release.md`. |
| [`Trial_And_Entitlement.md`](Trial_And_Entitlement.md) | **V1 SHIPPED — public 1.1.3** | Live Stripe Checkout + machine hash. Remaining: terms-acceptance record, cross-machine. Law: `docs/operations/Project_Laws.md` §Entitlement. |
...
| --- | --- | --- |
| [`Local_Runtime_Surface.md`](../archive/phases/Local_Runtime_Surface.md) | **CLOSED (2026-08-15)** — successor to packet 1. Wired discovery to `alln models` / `menu --json` `localRuntime` and the Mac Settings › CLIs LOCAL RUNTIME section. Live-verified on dogfood host at 1.1.19: pull → discover with no setup re-run; `enable --body` mints seat + repaired frozen `opencode.json`; doctor seated-only readiness; pinned free local no longer silently substituted to PAID. Full suite 4250/0; Visual Proof Gate PASS 9/9; GUI debt 0. | Law: `docs/operations/Project_Laws.md` §§Local Ollama seats, Local and cloud seats; vocab `Product_Vocabulary.md` §Local runtime; GUI brief `docs/gui/surfaces/local-runtime/brief.md`; code `OllamaLocalModelDiscoveryProvider`, `LocalRuntimeSeatMint`, `OllamaLocalDoctorReport`, `LocalRuntimeSeatDelete`, `LocalSeatPinHonesty`, `LocalRuntimeSectionView`, `MenuCatalog`, `ModelListProjector` |
...
| [`CLI_Install_Documents_TCC_Adhoc_Waive.md`](../archive/phases/CLI_Install_Documents_TCC_Adhoc_Waive.md) | **CLOSED — WAIVED (2026-08-11); first-user claim FALSIFIED 2026-08-13.** The waive said signed `curl \| sh` from `~` would not prompt. Founder measured Developer ID `1.1.3` from a Documents checkout: prompt on the one-liner. Cause: `get-alln.sh` exec'd `alln` with inherited Documents cwd; signing does not skip TCC, it only makes Allow stick. Belt: `cd "$HOME"` before any alln exec in `get-alln.sh` / uninstall. Live faucet still needs that script re-uploaded to R2. | Code SSOT: `scripts/get-alln.sh`, `scripts/uninstall-allnighter.sh`; `ProtectedCWDEscape` cannot avoid the first getcwd; debugger `docs/operations/debugger/2026-08-13-signed-cold-install-documents-tcc.md` |
...
| Ollama local seats (Claude Code and/or OpenCode; Available/Unavailable per seat); local models missing from the app or `alln models`; a newly pulled Ollama tag not appearing; which body runs a local model | Law: [`docs/operations/Project_Laws.md`](../operations/Project_Laws.md) §§Local Ollama seats, Local and cloud seats; vocab [`Product_Vocabulary.md`](../workflows/Product_Vocabulary.md) §Local runtime; GUI brief [`docs/gui/surfaces/local-runtime/brief.md`](../gui/surfaces/local-runtime/brief.md); code `OllamaLocalModelDiscoveryProvider`, `LocalRuntimeSeatMint`, `OllamaLocalDoctorReport`, `LocalSeatPinHonesty`, `MenuCatalog`, `ModelListProjector`. History: archived [`OpenCode_Local_Ollama_Seats.md`](../archive/phases/OpenCode_Local_Ollama_Seats.md), [`Local_Runtime_Surface.md`](../archive/phases/Local_Runtime_Surface.md) |
