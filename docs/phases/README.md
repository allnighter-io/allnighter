...
| [`Team_Depth_Naming.md`](Team_Depth_Naming.md) | **DECIDED — convention SSOT** (founder, 2026-07-16) | Family = the job, named by what it does (Spec Review, Bug Hunt). Depth = universal Min / (bare name) / Max; bare name is the default send; picker renders Min / Default / Max. Min/Max are Team-Lab-defined (necessity suite / forward selection), no numbers in names, rename map for the bug family included. Routing law: default send always → bare team, never Min. "Pressure Test" retired everywhere (marketing included). **Rename slice runs FIRST, before Team Lab** (cutover is DONE; lab artifacts must be born with final IDs). |
