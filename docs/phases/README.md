...
| [`Loop_Verb_Cutover.md`](Loop_Verb_Cutover.md) | **v5 LOCKED 2026-07-30 — ready for implementation, not started** | `alln pair relay\|pilot` → one `alln loop` verb: `start deliver … --pm <caller\|agent-id> --dev <id>`, plus `list\|status\|stop\|resume\|wait\|step\|pm`. Three laws: **kind names what *done* means**; **the PM chair is an occupant, not a mode**; **operations are defined against the state machine, never the chair**. Kills `relay`, `pilot`, `external`, `PMMode`, `--relay`, `--pm-model`, `--dev-model`, both `adopt`s; frees `alln pair` for device pairing. Contract 7.0.0 / binary 0.11.0; decode break accepted (finish in-flight loops before upgrading). **Supersedes `Product_Vocabulary.md` L27/L29/L51-53.** Mac: Delivery Loop. Sheet redesign deferred to §8; symbol sweep to LVC-S09. |
