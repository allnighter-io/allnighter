...
| [`Serve_Continuity.md`](Serve_Continuity.md) | **OPEN — v2; Bug Hunt `AC9D2295` landed; founder fork on login item** | Code red: orphan LA + rebuild-retired codesign identity. Fix: `ServeLifecycle` in `install-cli` transaction + fail-closed health + demand heal. No second scheduler. |
