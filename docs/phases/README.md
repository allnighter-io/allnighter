...
| Doc | Status | Purpose |
| --- | --- | --- |
| [`Ephemeral_Teams.md`](Ephemeral_Teams.md) | **Ready** — Option C locked (runtime `--seat` on `alln run`); no ephemeral-tag interim | One-off multi-seat staffing without catalog pollution. Agents pass `--seat` on `run`; `teams duplicate` stays library-only; founder `teams purge --unused` for existing throwaways. |
...
| One-off multi-seat staffing, custom-team picker sprawl, `teams duplicate` for throwaways | [`Ephemeral_Teams.md`](Ephemeral_Teams.md) — Ready; runtime `--seat` on `alln run` (Option C); no ephemeral-tag interim |
