...
| Doc | Status | Purpose |
| --- | --- | --- |
| [`CLI_Agent_Ergonomics.md`](CLI_Agent_Ergonomics.md) | **Draft 2026-07-20 — awaiting founder approval** — AE-S01–S10 | Successor to ASF: the CLI is honest, now make it **complete** (generated `--help`, no allowlist), **leak-free** (PO-F10 on the answer path; one choke point per law), and **free to probe** (`run --dry-run`, cwd→project, lab filter). Origin: cold-agent dogfood where 7 of 11 claims were false because the surface under-reports the product. |
