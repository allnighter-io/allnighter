...
| [`One_Paste_Cold_Start.md`](One_Paste_Cold_Start.md) | **OPEN — founder priority (distribution)** | Cold install + **shared release channel**: curl one-liner → PATH + host paste; `latest.json` + `menu.update` so agents/CLI-only humans learn about *releases* without opening the Mac app; Mac update UI uses the same channel (not Sparkle-only SSOT). **No MCP.** BQ-1/2 block public; S06 update discovery. Slices OPC-S00–S06. |
