...
| [`Loop_Verb_Cutover.md`](Loop_Verb_Cutover.md) | **Draft — founder approved 2026-07-30, not started** | `alln pair relay\|pilot` → `alln loop delegate\|pilot`. Law: **verbs are lifetimes, nouns are seatings** (`run`/`loop` vs `team`/`seat`/`model`). Hard cutover via `RetiredVocabulary` gate; contract 7.0.0 / binary 0.11.0; frees `alln pair` for device pairing; Mac offers Delegation Loop only. **Supersedes `Product_Vocabulary.md` L27/L51-53** (the never-rename-`pair loop` rule). Sheet redesign deferred to §7. |
