...
| [`Agent_Team_Loop.md`](Agent_Team_Loop.md) | **OPEN — Ready for Implementation** | Mac Compose **Loop** = unattended Relay with kickoff handoff message; remove project spinner; CLI parity for `--message` + `pair relay stop` + Status/Stop in thread. Pilot stays CLI-only. Reuses PM Turn Delivery / Round Survives substrate. |
...
| Mac Compose Loop / Agent Team Loop; remove relay spinner; kickoff handoff; Stop+Status parity with CLI | [`Agent_Team_Loop.md`](Agent_Team_Loop.md) — OPEN; Pilot remains CLI-only |
