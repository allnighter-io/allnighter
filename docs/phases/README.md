...
| [`Team_Catalog_Normalization.md`](Team_Catalog_Normalization.md) | **Specced v1** (prerequisite for the intent router) | Decisive normalized family list: obvious job names, and tiers (Min/Default/Max) OPTIONAL per family — like effort levels (Spec Review/Bug Hunt/Growth/Design tiered; Copy/Security/Signal single). Renames Code Core→Plan, Premium Polish→Polish, etc.; drops Conversion Studio; adds Bug Hunt Min. |
| [`Agent_Intent_Router.md`](Agent_Intent_Router.md) | **Specced v1** (gate 3 — routes intent → right team) | `alln team hello --for "<intent>"` becomes a local solutions engineer over the normalized catalog. **`Team_Catalog_Normalization.md` lands first** (hard prerequisite); router matches on `typeTags`/`description`, deterministic, no silent swap. |
