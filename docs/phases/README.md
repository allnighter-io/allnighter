...
| Doc | Status | Purpose |
| --- | --- | --- |
| [`Worker_Skill_Sharing.md`](Worker_Skill_Sharing.md) | **Brainstorm — seat/skill direction locked** | Shared skills across teams; **seat** = model + skill (not “worker”); staff-seat UI (model → skill → skill.md); kill fork-on-save + skill overrides (B1). Shipped adjacent: Skills tab removed, `alln skills gc`. |
