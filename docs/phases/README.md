...
| [`Agent_Team_Loop.md`](Agent_Team_Loop.md) | **OPEN — Ready for Implementation** (Grok adversarial freeze 2026-07-29) | Mac Compose **Loop** = unattended **detached** Relay; kickoff via `--message`/`--message-file` → `kickoffMessage` → first PM prompt; `pair relay stop` (founder stopped + PM Turn); remove spinner; Status/Stop CLI parity; Pilot CLI-only. |
...
| Mac Compose Loop / Agent Team Loop; remove relay spinner; kickoff handoff; Stop+Status parity with CLI | [`Agent_Team_Loop.md`](Agent_Team_Loop.md) — Ready (Grok adversarial freeze 2026-07-29); Pilot CLI-only; Mac start must detach |
