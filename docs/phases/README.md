...
| --- | --- | --- |
| [`Local_Runtime_Surface.md`](Local_Runtime_Surface.md) | **CLOSED 2026-08-15 — ready to archive** | Packet 1 shipped a working local Ollama seat pipeline and never wired it to a command or a screen: `discover()`, `ModelDiscoveryRegistry.provider(for:)` and `assessExplicitEnable()` all had ZERO production call sites; `alln models` listed 43 seats and 0 local; the Mac app contained no Ollama string at all. Now live-verified end to end on the dogfood host at 1.1.19 — pull a tag and it appears with no setup re-run, `enable --body` mints a seat and repaired a config frozen since 08-13, doctor stopped calling unseated tags Available, and a pinned free local seat is no longer silently substituted onto a PAID one. Full suite 4250/0. Visual Proof Gate PASS 9/9. GUI debt 0. |
