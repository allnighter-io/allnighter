...
1. **Superseded by `docs/phases/Loop_Verb_Cutover.md` (2026-07-30):**
   the CLI verb is `alln loop` (renamed from `pair relay*`/`pair pilot`); the
   `Loop` row in `docs/workflows/Product_Vocabulary.md` already reflects this.
   Do not re-add the old CLI verb as an alias.
2. Add / extend the **Agent Team Loop** help topic: Mac Loop = unattended
   `alln loop`, kickoff brief, `alln loop stop` vs `alln kill`.
