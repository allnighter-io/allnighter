...
> current-state inventory, not an implementation plan.

Status: **TOMBSTONED / DO NOT IMPLEMENT — superseded by Code Red.**
Historical status at freeze: REB-S01–REB-S04 and several CPH patches had
landed, but the real protected-project workflow remained red.
...
- source probes are coordinator-scoped and use Allnighter's neutral scratch
  ...
  A client may contribute only a validated, already-computed Git commit ID for
  binary-freshness comparison; the coordinator never resolves that ID back to a
  checkout or reads the caller's workspace;
