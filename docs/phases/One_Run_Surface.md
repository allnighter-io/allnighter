...
## Works Test — host 2 (Cursor)

Codex (the host named in the packet) is DEFERRED — capacity 0% and host-sandbox FS_PERMISSION_DENIED on 2026-08-01; re-run required before the two-host Done-when line can be ticked.

Run id `0D8E1497-10F4-4675-A62C-C8EA887AC726`; binary on PATH; contract **9.3.0**. Founder-approved Cursor substitution; this does **not** satisfy the packet's named two-host set (Codex + one other).

1. **PASS** — `alln menu --json` exited 0 without crashing; observed `contractVersion: "9.3.0"`.
2. **PASS** — Exact acknowledgement: `{"id":"0D8E1497-10F4-4675-A62C-C8EA887AC726","kind":"run","nextAction":{"command":"alln show 0D8E1497-10F4-4675-A62C-C8EA887AC726 --stream","kind":"showRun"},"pid":69052,"status":"dispatched"}`; exactly one `nextAction`, no `delivery`, no absolute binary path.
3. **FAIL** — First live `alln show <id> --stream` exited **0** and emitted frames in order: `teamRunSnapshot` → `teamRunStarted` (`replayed: true`) → `workerStarted` (`replayed: true`) → `workerActivity` → `workerAnswered` → `planStarted` → `planWritten` → exactly one `teamRunCompleted` (live frames omitted `replayed`). (a) First frame was `teamRunSnapshot`; `observation` had exactly three keys `ownerState`, `activityMode`, `lastActivityAt`. (b) **FAIL** — `lastActivityAt` was `null` on the first snapshot (`ownerState: "alive"`, `activityMode: "incremental"`) and remained `null` on the terminal frame (`ownerState: "dead"`) — neither an ISO8601 string nor a raw number. (c) PASS — replayed frames carried `replayed: true`; live frames omitted it. (d) PASS — one `workerActivity` with `activityKind: "tool"` and `tool: "read_file"`. (e) PASS — exactly one `teamRunCompleted`. (f) exit **0**. Step marked FAIL because (b) failed.
4. **FAIL** — Re-running the same `alln show 0D8E1497-10F4-4675-A62C-C8EA887AC726 --stream` emitted `teamRunSnapshot` then `error` (`JOURNAL_CORRUPT`, message `event sequence gap: expected seq 1248, got 1249`), **zero** terminal frames, exit **1**.
5. **PASS** — Durable journal kinds in order: `run.status_changed`, `worker.status_changed`, `worker.tool` (`read_file`), `worker.status_changed`, `stage.started`, `stage.completed`, `run.status_changed`. `worker.tool` present; no transcript kinds (`worker.answer_delta`, `worker.reasoning_delta`, `worker.output`, `stage.output`). Surprising: seq gap `1247 → 1249` (missing `1248`) — same hole that broke step-4 reattach.
6. **PASS** — `alln team status <id> --json` and `alln team result <id> --json` each exited **2** with `CLI_USAGE_ERROR`; messages named `alln show`; neither executed anything.
7. **PASS** — Did not use `alln ps`, `git status`, a polling loop, or any private path except the explicitly authorized step-5 journal audit.
