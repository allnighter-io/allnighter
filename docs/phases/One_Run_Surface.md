...
Re-run 2026-08-01 against fixed binary **0.12.1** / contract **9.3.0** (`alln 0.12.1 (contract 9.3.0, hash c300413efa8e…, git 67e7e25f070d)`). Main run `DEC6A16A-586D-4932-9FC7-4C0156922EAA`; concurrency idA `9C1E0982-29D2-4A1A-842D-911BDF9519A9`, idB `460003E7-7C4E-4D02-A17D-49461C53CF01`. Founder-approved Cursor substitution; this does **not** satisfy the packet's named two-host set (Codex + one other). Done-when two-host line stays unticked.

1. **PASS** — `alln menu --json` exited 0; observed `contractVersion: "9.3.0"`.
2. **PASS** — Exact acknowledgement: `{"id":"DEC6A16A-586D-4932-9FC7-4C0156922EAA","kind":"run","nextAction":{"command":"alln show DEC6A16A-586D-4932-9FC7-4C0156922EAA --stream","kind":"showRun"},"pid":65918,"status":"dispatched"}`; exactly one `nextAction`, `kind: showRun`, `alln show <id> --stream`, no `delivery`, no absolute path.
3. **PASS** — Immediate attach (no sleep); exit **0**. (a) First frame `teamRunSnapshot`; `observation` exactly three keys `ownerState`, `activityMode`, `lastActivityAt`. (b) First snapshot `lastActivityAt: null` (explicit); terminal `"lastActivityAt":"2026-08-02T05:38:04Z"` — never raw number, never omitted. (c) `teamRunStarted`/`workerStarted` carried `replayed: true`; live frames from `workerActivity` seq 1583 onward omitted it. (d) `workerActivity` with `activityKind: "tool"` and `tool: "read_file"`. (e) Exactly one `teamRunCompleted` (seq 1587). (f) exit **0**.
4. **PASS** — Same `alln show DEC6A16A-586D-4932-9FC7-4C0156922EAA --stream` on terminal run: replayed frames with `replayed: true`, exactly one `teamRunCompleted` (seq 1587), no error / no `JOURNAL_CORRUPT`, exit **0**.
5. **PASS** — Concurrent idB dispatched while streaming idA; first snapshot `status: running` / `ownerState: alive`; **no `JOURNAL_CORRUPT`**; reached `teamRunCompleted` seq 1600; exit **0**. Seq gaps on idA stream: `1589→1592` (missing 1590–1591), `1592→1597` (missing 1593–1596); then contiguous `1597→1600`.
