...
| LLM judges exhibit measurable self-preference bias (−38% to +90% on ArenaHard) and position bias; position bias is mitigable by rearranging option order. | [Self-Preference Bias in LLM-as-a-Judge](https://arxiv.org/abs/2410.21819) | The resolver must not have authored either claim, and claim order must be randomised. Backs the existing source-blind synthesis rule with a number. |
