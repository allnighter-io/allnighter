# Review Workflow — Allnighter (iOS) · the floor manager

The review workflow on iPhone — **glance and decide**, not full editing. The
phone creates the habit; the Mac (`ui_kits/review/`) creates the power. Same
...
## Views (complete · 10)
Inbox · Compose + Call Plan · Live run · Plan writer Analysis · Review Board ·
...
- **Inbox / "Needs you"** — runs awaiting a decision (final spec ready,
  blocker on the board, a return to plan writer) + what's running, with a progress bar.
...
Outcome · Routing · Live run · Plan writer Analysis · Scorecards.
...
- `screens3.jsx` — Routing, Live run, Plan writer analysis, Scorecards; the final `JIOSApp` (full detail flow incl. routing/analysis/scorecards/liverun).
