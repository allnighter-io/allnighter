# Review Workflow — Allnighter (macOS) · the RB milestone

A high-fidelity, click-through recreation of the **Review Workflow / Review
Board** capability (specs `uploads/RB0–RB5`). It evolves the Team from "one
prompt → plan" into a full control loop:

```
prompt → team → plan writer analysis → draft plan → review board → final spec
...
## Views in this batch (1 of N)- **① Composer + Call Plan** (`prompt`) — pick the workflow preset
  (synthesis_only / light_review / full_review), configure team **seats** with
  per-worker **stances** (self-fusion), pick the plan writer, and watch the live
...
- **⑤ Final Spec** (`final`) — the decision-grade `final_spec.md`: an
  **executability** banner (Works Test + proof commands present), required
  ...
  deferred) on review feedback and team contradictions. "Implement this" hands
  off to dispatch.

The other rail stages (Team, Plan writer analysis, Draft plan, Dispatch, Return
review) show an "Up next" stub — they're the next mockup batches.

## Files
- `index.html` — mounts `ReviewApp` (rail nav + active view).
