...
/**
 * A worker in the panel or live run grid: brand glyph, name, model ID, and run status.
 * Selectable (panel) or read-only (run view).
 * @startingPoint section="Product" subtitle="Worker row — panel & live run" viewport="700x150"
 */
export interface WorkerChipProps extends React.HTMLAttributes<HTMLElement> {
  ...
  /** Panel mode: show a selection checkbox. */
  selectable?: boolean;
  selected?: boolean;
  onToggle?: () => void;
}
