...
- **Shape before commit.** Every tool result reports observed `invocations`; a governor caps
  concurrent teams; the CLI/MCP lists preset **work shape** via `WorkOrder.summary`
...
| `team_ask` | Run a team and return the result. Optional `waitSeconds` is a **client timeout** only — may return a `runId` to poll; never branches on a predicted duration. |
| `team_start` | Kick off a team asynchronously; return a `runId` immediately. |
| `team_status` | `{ runId } -> { status, workersDone/total, stage, invocations }`. |
...
`TeamGovernor` caps concurrent teams at `maxConcurrentTeamRuns` across
