...
team→plan writer→plan pattern **locally, on the user's machine, through CLIs they
already pay a flat rate for — zero marginal cost.**

RB6 turns that into the unfair advantage: **expose Allnighter's team as a local
...
Allnighter fans the question out to the local team, runs the structured plan writer,
...
> Fusion is a feature you rent. Allnighter becomes the review layer the whole
...
Expose the team as a tool any local agent can invoke, over three transports
...
Review in, spec out. The tool **never writes code, runs git, or dispatches an
...
- **No git, worktrees, branches, commits, landing, or code execution.** RB6 is
  *review only.* Direct executor dispatch stays the separate, explicit, GUI-gated
...
- **No network egress** beyond `localhost` and the CLIs' own auth. No remote/
  ...
  seam, not this.)
- **No general RPC / no DAG.** A fixed team tool surface, mirroring the rest of
...
- **Recursion is impossible.** A team worker can never trigger a nested team
...
- **Cost is never silent.** Every tool result reports `callsSpent`; a governor caps
  concurrent teams; the CLI/MCP can preview a `CallPlan` before committing.
...
- **Review only.** The tool has no capability to write files, run git, or invoke
  the executor. Capability boundary enforced in code, not just convention.
- **Graceful degradation.** Partial teams and unhealthy workers are reported
...
```text
   ...
   shell: `alln team`   MCP stdio (JSON-RPC)   HTTP/WS (127.0.0.1 + token)
        ...
        +----------+-----------+------------+------------+
                   ...
                   +------------+-------------+
                                |
            existing engine: TeamRunCoordinator + Plan writer (06)
                                |
...
server, and app run teams together or apart.
...
| Operation | Purpose |
| --- | --- |
| `team_presets` | List exposed presets + each one's `CallPlan` estimate (calls, est. time, quota risk — labeled estimates). |
| `team_ask` | Run a team and return the result. **Self-correcting sync:** if the preset's `CallPlan` estimate exceeds `waitSeconds`, it returns a `runId` + `estimatedSeconds` immediately instead of blocking into a client timeout. |
| `team_start` | Kick off a team asynchronously; return a `runId` immediately. |
| `team_status` | `{ runId } -> { status, workersDone/total, stage, callsSpent }`. |
| `team_result` | `{ runId } -> TeamToolResult`. |
| `team_recall` | **Read-only**, zero-cost: search prior teams and return past reviews (with dates). The team *remembers* — reuse a prior decision without spending a call. |

Sync vs async is deliberate and **self-correcting**: `team_ask` compares the
...
carries `createdAt`** so the agent can plan writer staleness (acting on a 6-month-old
...
- partials: [WorkerFailure] # honest: which workers didn't answer
...
- waitSeconds: Int?                       # team_ask sync budget
...
- exposedPresetIds: [String]              # default = the Phase 06 team presets ONLY (no review/final until RB1-3)
...
truncated. Until RB1–RB3 land, `exposedPresetIds` defaults to the Phase 06 team
...
A team's own worker/plan writer CLIs (e.g. `claude`) may have the Allnighter tool
registered globally. Without a guard, a worker calls the team → spawns workers →
call the team — infinite recursion and quota detonation. The guard must hold for
...
- **Env, for spawned clients.** Every worker subprocess is spawned with
  ...
  `allowSinglePassthroughAtDepth`, return one direct answer). Never a nested team.
- **HTTP fails closed.** The loopback server cannot read a caller's env, so every
  team-start request **must** carry an `X-Allnighter-Team-Depth` header; all
...
`CouncilGovernor` caps concurrent teams at `maxConcurrentCouncils` across
...
crashed team never leaves a stale lock; a belt-and-suspenders TTL + PID-liveness
...
reports `callsSpent`; `team_presets` previews the `CallPlan`.
...
- **Capability boundary = no writes outside `AllnighterPaths`.** The tool legitimately
  ...
  dispatch.** The tool target links the team engine only (no dispatch/executor
...
- [ ] RB6-S03 - **Recursion guard (fail-closed):** inject `ALLNIGHTER_COUNCIL_DEPTH`
  ...
  worker cannot start a nested team on any transport.
...
for the run store?") with the Fast Team preset. Allnighter runs the local team
(zero API cost), returns a plan + structured PlanAnalysis + callsSpent;
the agent uses it and continues — no window switch, no clipboard.

Prove the guards:
- The team's own `claude` worker, with the tool registered, attempts a nested
  team_ask on each transport -> refused (env depth for CLI/MCP; fail-closed
  depth header for HTTP). No second team, no quota burn.
- A crashed CLI mid-team leaves no stale lock (flock auto-release); the next
  team proceeds.
...
- `team_recall("run store")` returns the earlier review (with its date), 0 calls.
...
- [ ] A team worker cannot start a nested team on **any** transport
...
- [ ] Every tool result reports `callsSpent`; `team_presets` previews a `CallPlan`;
  `team_ask` auto-returns a runId when the estimate exceeds `waitSeconds`.
...
Allnighter is now the **local review layer for the whole machine**: any agent the
user runs can summon a zero-cost, Fusion-grade team mid-task and inherit the
