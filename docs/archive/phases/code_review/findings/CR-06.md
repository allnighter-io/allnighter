...
### P2 — `.failure` planner takeover context lacks `lastWorkerOutput` and uses stale check data
- **Invariant (takeover quality):** the planner should get the richest available
  context to diagnose the failure.
- **Evidence:** the `.failure` takeover context (`PairCoordinator.swift:237-242`)
  passes `checkExitCode: entry.checkExitCode` and `checkStdoutTail:
  entry.lastStdoutTail` — stale fields from a *previous* successful iteration
  (only updated on `.success` at `:274-275`). If the first attempt fails, both
  are nil. It also omits `lastWorkerOutput` entirely, while the `.stalled`/
  `.failed` path (`:310-316`) includes `lastWorkerOutput:
  outcome.childRun?.workerAnswers.first?.output`. The planner gets strictly less
  context when the run service itself fails than when the run produces a failed
  terminal.
- **Suggested fix:** if `runSlice` can surface partial run/worker data on
  `.failure`, pass it through; otherwise document that `.failure` takeover
  context is intentionally sparse.
- **Suggested slice:** (nit, fold into the next touch of this file)

### P2 — Planner takeover + counting block is duplicated across `.failure` and `.stalled`/`.failed`
- **Invariant (maintainability):** the post-takeover state update and counter
  accounting should have one owner.
- **Evidence:** the `.failure` takeover (`PairCoordinator.swift:231-252`) and
  the `.stalled`/`.failed` takeover (`PairCoordinator.swift:304-326`) are
  structurally identical: call `runPlannerTakeover`, `applyPlannerTakeover`,
  then the same `if entry.status == .passed { if entry.resolvedBy == "planner"
  { plannerResolved += 1 } else { passed += 1 } } else { escalated += 1 }` +
  `settled = true` block (`:246-251` and `:320-325`). The only real difference
  is the `PlannerTakeoverPrompt.Context` initializer (`:237-242` vs
  `:310-316`). A future change to the counting logic must be made in both
  places or they drift.
- **Suggested fix:** extract a helper that takes the context as input and
  performs the takeover + counting + `settled = true`, so the accounting has
  one source of truth.
- **Suggested slice:** (nit, fold into the next touch of this file)

### P2 — `infraBackoff` uses a hardcoded 5 s sleep with no exponential backoff
- **Invariant (liveness/decency):** during a sustained infra outage, the queue
  should not hammer the infra at a fixed 5 s cadence.
- **Evidence:** `.infraBackoff` sleeps exactly `5_000_000_000` ns
  (`PairCoordinator.swift:290`) on every iteration, with no backoff multiplier.
  Combined with the unbounded retry (P1 above), a sustained outage produces a
  tight 5 s poll loop for the entire duration.
- **Suggested fix:** exponential backoff with a cap (e.g. `min(5 * 2^n, 60)`)
  or fold into the infra-backoff retry cap from the P1 fix.
- **Suggested slice:** (nit, fold into the infra-backoff cap slice)
