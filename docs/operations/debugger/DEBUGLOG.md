...
## 2026-07-30 - Cancel-lie recovery + `alln ps` force-killed the caller's own process group

Tier: T2 (regression introduced by the cancel-lie fix in `fbf1e4b8`)
Symptom: `swift test --filter KillSettlementTests` died mid-suite with exit 0 and no failure line — the XCTest runner itself was being SIGKILLed by the code under test at `testContradictionSurfacesAfterClockKilledWithSurvivor`.
Truth owner: `RunStore.reconcileRunDetailed` owns whether a reconcile signals; `ProcessOwnershipSurface.list` is contractually read-only ("`ps` never kills — this is the read-only would-reap report").
Lie-prone layer: The cancel-lie recovery branch was added to `reconcileRunDetailed` unconditionally. `ProcessOwnershipSurface.list` reconciles on read (CLP-S02) and `AsyncTeamService.status`/`result` reconcile too, so every read path inherited a TERM→SIGKILL of the recorded pgid of any terminal run with a live recorded member. In the tests the recorded member is this process, so `ps` killed the runner. In production a foreground `alln run` records its own group, so a concurrent `alln ps` could kill the user's shell foreground group.
RCA: Two separate defects. (1) The recovery action was placed in the shared reconcile primitive instead of the explicit operator path, so a read surface acquired a kill. (2) The new `ProcessOwnership.anyRecordedMemberIdentityAlive` counted a non-PG-killable `inProcess` coordinator as live ownership, contradicting `RunContradictionSurface.contradiction`, which documents that an in-process coordinator "is the reader/app itself and must never trip the contradiction." That made every completed in-process run a permanent cancel lie: `WOULD_REAP` forever and `killOutcome` restamped `.partial` over a genuinely stopped tree.
Fix boundary: Gate recovery behind `recoverTerminalLiveOwnership`, default `false`, on `reconcileRun`/`reconcileRunDetailed`/`reconcileAll`; only `AsyncTeamService.reconcile` (explicit `alln team reconcile`) passes `true`. Align `anyRecordedMemberIdentityAlive` with the contradiction rule by requiring `isProcessGroupKillable`. The reporting half of the original fix (`wouldReconcile`, `ps` contradiction, cancel's own recovery branch) is unchanged — detection was correct, only the action was misplaced.
Attempt count: 1 (the fix Grok aborted mid-flight).
Seam: read surfaces (`ps`/`status`/`result`) -> `reconcileAll` -> `reconcileRunDetailed` -> `forceTerminateAllRecorded` -> recorded pgid.
