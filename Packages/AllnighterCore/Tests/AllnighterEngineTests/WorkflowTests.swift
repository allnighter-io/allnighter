...
final class WorkflowTests: XCTestCase {
    ...
    func testValidPresetOrderPasses() throws {
        let preset = WorkflowPreset(id: "p", displayName: "p", workerSpecs: [SeatSpec(modelId: "model_opus")], synthesis: config(), stages: [reviewStage(), finalStage()])
        XCTAssertNoThrow(try preset.validate())
    }

    func testReviewAfterFinalIsRejected() {
        let preset = WorkflowPreset(id: "p", displayName: "p", workerSpecs: [SeatSpec(modelId: "model_opus")], synthesis: config(), stages: [finalStage(), reviewStage()])
        XCTAssertThrowsError(try preset.validate())
    }

    func testMultipleFinalStagesRejected() {
        let preset = WorkflowPreset(id: "p", displayName: "p", workerSpecs: [SeatSpec(modelId: "model_opus")], synthesis: config(), stages: [finalStage(), finalStage()])
        XCTAssertThrowsError(try preset.validate())
    }
    ...
    func testWorkflowPresetStoreValidatesOnSave() throws {
        ...
        let good = WorkflowPreset(id: "g", displayName: "g", workerSpecs: [SeatSpec(modelId: "model_opus")], synthesis: config(), stages: [reviewStage(), finalStage()])
