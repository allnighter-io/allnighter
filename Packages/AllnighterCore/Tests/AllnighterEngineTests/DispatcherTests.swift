...
final class DispatcherTests: XCTestCase {
    ...
    private func runWithFinalSpec() -> TeamRun {
        var run = TeamRun(id: "r", prompt: "Build X", status: .complete,
                             workers: [TestSupport.seat("model_opus")],
                             workerAnswers: [WorkerAnswer(workerId: "model_opus#0", modelId: "model_opus", status: .done, output: "a")],
                             createdAt: Date())
        run.stages = [
    ...
    func testBriefPrefersFinalSpec() {
        let brief = BriefBuilder.build(run: runWithFinalSpec(), executionWorkerId: "model_opus", workingDirectory: "/tmp")
    ...
    func testBriefFallsBackToMasterPlanLessReviewed() {
        ...
        let brief = BriefBuilder.build(run: run, executionWorkerId: "model_opus", workingDirectory: "/tmp")
        XCTAssertEqual(brief?.sourceArtifact, .plan)
    ...
    func testRevealOnlyWritesArtifactsWithoutInvoking() async {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: tmp.appendingPathComponent("execution_prompt_model_opus_01.md").path))
    }
