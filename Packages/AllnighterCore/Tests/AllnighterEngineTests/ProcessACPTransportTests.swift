...
final class ProcessACPTransportTests: XCTestCase {
    ...
    func testLiveWarmTurnsRecallAndStayFast() async throws {
        ...
        let t0 = Date()
        try await session.start(cwd: cwd)
        print("ACP start (handshake + session/new): \(Int(Date().timeIntervalSince(t0) * 1000))ms")

        let t1 = Date()
        let a1 = try await answer(session.prompt("Remember the word amberclock. Reply with just: OK"))
        print("turn 1: \(Int(Date().timeIntervalSince(t1) * 1000))ms answer=\(a1.prefix(40))")

        let t2 = Date()
        let a2 = try await answer(session.prompt("What single word did I ask you to remember? Reply with ONLY that word."))
