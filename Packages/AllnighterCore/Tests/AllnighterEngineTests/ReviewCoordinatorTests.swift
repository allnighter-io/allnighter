...
final class ReviewCoordinatorTests: XCTestCase {
    ...
    private func completedRun() -> TeamRun {
        var run = TeamRun(id: "r", prompt: "Build X", status: .complete,
                             ...
                             createdAt: Date())
        run.stages = [
            StageOutput(id: "a", purpose: .analysis, status: .done, payload: .analysis(PlanAnalysis(consensus: [AnalysisPoint(statement: "concurrency matters")]))),
    ...
    func testReviewsRenderInBundleAndStore() throws {
        ...
        let bundle = RunMarkdown.bundle(run, models: [])
        ...
        let dir = try RunStore(rootDirectory: tmp).save(run, models: [])
