...
public enum DefaultConfig {
    ...
    private static let manifestJSON: [String] = [
        ...
        #"{"id":"grok","manifestVersion":1,"displayName":"Grok Build CLI","kind":"headless_cli","detectCommand":"grok --version","smokeTestCommand":"grok -p \"Reply with the single token ALLNIGHTER_READY\" -m {{model}} --output-format plain","smokeTestExpect":"ALLNIGHTER_READY","invoke":{"command":"grok","args":["-p","{{prompt}}","-m","{{model}}","{{effortArgs}}","--output-format","streaming-json","--always-approve","--no-wait-for-background","--no-subagents","--disable-web-search","--cwd","{{workingDir}}"],"promptVia":"arg","env":{},"workingDir":null,"timeoutSeconds":300,"effortFlag":{"flag":"--reasoning-effort","levels":{"low":"low","med":"medium","high":"high"}}},"output":{"capture":"stdout","stripAnsi":true,"doneSignal":"exit_code","sentinel":null},"streaming":{"supported":true,"mode":"jsonl_stdout","args":[],"partialOutput":true,"answerDeltaPaths":["$.data"],"finalAnswerSource":"parser_accumulator","stripAnsi":true,"visibleReasoning":false},"session":{"continuity":"vendor_session","acquire":"capture","resumeArgs":["-p","{{prompt}}","-m","{{model}}","{{effortArgs}}","--output-format","streaming-json","--always-approve","--no-wait-for-background","--no-subagents","--disable-web-search","--cwd","{{workingDir}}","--resume","{{sessionId}}"],"capture":{"from":"stream_json","field":"sessionId"}}}"#,
