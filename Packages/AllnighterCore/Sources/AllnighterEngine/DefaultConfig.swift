...
public enum DefaultConfig {
    ...
    private static let manifestJSON: [String] = [
        ...
        #"{"id":"grok","manifestVersion":1,"displayName":"Grok Build CLI","kind":"headless_cli","detectCommand":"grok --version","smokeTestCommand":"grok -p \"Reply with the single token ALLNIGHTER_READY\" -m {{model}} --output-format plain","smokeTestExpect":"ALLNIGHTER_READY","invoke":{"command":"grok","args":["-p","{{prompt}}","-m","{{model}}","{{effortArgs}}","--output-format","streaming-json","--always-approve","--no-wait-for-background","--no-subagents","--disable-web-search","--cwd","{{workingDir}}"],"promptVia":"arg","env":{},"workingDir":null,"timeoutSeconds":1800,"effortFlag":{"flag":"--reasoning-effort","levels":{"low":"low","med":"medium","high":"high"}}},"output":{"capture":"stdout","stripAnsi":true,"doneSignal":"exit_code","sentinel":null},"streaming":{"supported":true,"mode":"jsonl_stdout","args":[],"partialOutput":true,"answerDeltaPaths":["$.data"],"finalAnswerSource":"parser_accumulator","stripAnsi":true,"visibleReasoning":false},"session":{"continuity":"vendor_session","acquire":"capture","resumeArgs":["-p","{{prompt}}","-m","{{model}}","{{effortArgs}}","--output-format","streaming-json","--always-approve","--no-wait-for-background","--no-subagents","--disable-web-search","--cwd","{{workingDir}}","--resume","{{sessionId}}"],"capture":{"from":"stream_json","field":"sessionId"}}}"#,
        #"{"id":"antigravity","manifestVersion":1,"displayName":"Antigravity","kind":"headless_cli","maxConcurrentSpawns":1,"detectCommand":"agy --version","smokeTestCommand":"agy --print \"Reply with the single token ALLNIGHTER_READY\" --model \"{{model}}\" --dangerously-skip-permissions","smokeTestExpect":"ALLNIGHTER_READY","invoke":{"command":"agy","args":["--print","{{prompt}}","--model","{{model}}","--dangerously-skip-permissions"],"promptVia":"arg","env":{},"workingDir":null,"timeoutSeconds":300},"output":{"capture":"stdout","stripAnsi":true,"doneSignal":"exit_code","sentinel":null},"session":{"continuity":"vendor_session","acquire":"capture","resumeArgs":["--print","{{prompt}}","--model","{{model}}","--dangerously-skip-permissions","--conversation","{{sessionId}}"],"capture":{"from":"session_dir","dir":"~/.gemini/antigravity-cli/brain"}}}"#,
        #"{"id":"cursor_agent","manifestVersion":1,"displayName":"Cursor Agent","kind":"headless_cli","maxConcurrentSpawns":1,"detectCommand":"agent --version","smokeTestCommand":"agent -p --output-format text --model composer-2.5 --trust \"Reply with the single token ALLNIGHTER_READY\"","smokeTestExpect":"ALLNIGHTER_READY","invoke":{"command":"agent","args":["-p","--output-format","text","--model","{{model}}","--trust","--workspace","{{workingDir}}","{{prompt}}"],"promptVia":"arg","env":{},"workingDir":null,"timeoutSeconds":300},"output":{"capture":"stdout","stripAnsi":true,"doneSignal":"exit_code","sentinel":null},"streaming":{"supported":true,"mode":"jsonl_stdout","args":["-p","--output-format","stream-json","--stream-partial-output","--model","{{model}}","--trust","--workspace","{{workingDir}}","{{prompt}}"],"partialOutput":true,"answerDeltaPaths":["$.message.content[*].text"],"finalAnswerSource":"event","stripAnsi":true,"visibleReasoning":false},"setup":{"bins":["agent","cursor-agent"],"knownPaths":["~/.local/bin","/opt/homebrew/bin","/usr/local/bin"],"installHint":"Install Cursor CLI with `curl https://cursor.com/install -fsS | bash`, then run `agent` or `agent login` if needed.","docsURL":"https://cursor.com/docs/cli/installation","loginFlow":{"interactiveCommand":"agent login","instructions":"Run `agent login` and complete Cursor sign-in. If account-backed commands fail with Keychain errors, open Cursor once and retry setup.","authErrorPatterns":["SecItemCopyMatching failed","not authenticated","not signed in","login","unauthorized","401"],"docsURL":"https://cursor.com/docs/cli/using"},"headlessTrust":{"required":true,"cliFlag":"--trust","disclosure":"Cursor Agent workers run with --trust: the agent may edit files, run shell commands, and use tools in the project workspace without per-run approval. This is required for headless Code workers and is separate from Bench or model enablement."}},"session":{"continuity":"vendor_session","acquire":"capture","resumeArgs":["-p","--output-format","stream-json","--stream-partial-output","--model","{{model}}","--trust","--workspace","{{workingDir}}","--resume","{{sessionId}}","{{prompt}}"],"capture":{"from":"stream_json","field":"session_id"}}}"#,
        #"{"id":"opencode","manifestVersion":1,"displayName":"OpenCode","kind":"headless_cli","maxConcurrentSpawns":1,"detectCommand":"opencode --version","smokeTestCommand":"opencode run --attach http://127.0.0.1:4096 --dir \"{{workingDir}}\" \"Reply with the single token ALLNIGHTER_READY\" --dangerously-skip-permissions -m \"{{model}}\"","smokeTestExpect":"ALLNIGHTER_READY","invoke":{"command":"opencode","args":["run","--attach","http://127.0.0.1:4096","--dir","{{workingDir}}","{{prompt}}","--dangerously-skip-permissions","-m","{{model}}"],"promptVia":"arg","env":{},"workingDir":"{{workingDir}}","timeoutSeconds":600},"output":{"capture":"stdout","stripAnsi":true,"doneSignal":"exit_code","sentinel":null},"streaming":{"supported":true,"mode":"jsonl_stdout","args":[],"partialOutput":true,"answerDeltaPaths":[],"finalAnswerSource":"parser_accumulator","stripAnsi":true,"visibleReasoning":false},"setup":{"bins":["opencode"],"knownPaths":["~/.local/bin","/opt/homebrew/bin","/usr/local/bin"],"installHint":"Install with `brew install opencode`. Add your Featherless/OpenAI API key in ~/.config/opencode/opencode.json. Allnighter starts the local OpenCode server automatically during setup and runs — no separate terminal step.","docsURL":"https://opencode.ai/docs/cli/","authErrorPatterns":["api key","authentication","unauthorized","401","rate limit","model is busy"]}}"#,
