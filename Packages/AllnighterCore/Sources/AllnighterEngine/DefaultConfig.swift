...
public enum DefaultConfig {
    ...
    public static func tieredPresets(models: [Model]) -> [PanelPreset] {
        ...
        func specs(_ ws: [Model]) -> [ModelSpec] { ws.map { ModelSpec(modelId: $0.id) } }
        ...
        if let strongest {
            presets.append(PanelPreset(id: "preset_self_double", displayName: "Self-Double", workerSpecs: [ModelSpec(modelId: strongest.id, count: 3)], synthesis: config(.combined), builtIn: true))
        }
        return presets
    }
}
