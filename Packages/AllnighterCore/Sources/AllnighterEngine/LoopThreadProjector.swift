...
/// collision-free `relay_<uuid>` string, `LoopCoordinator.idFactory`). That's the
/// "metadata slot" that carries the relay's provenance — no new field on `WorkThread` or
/// `LoopState`, so this stays a projection-only slice with zero new durable primitives
...
/// than a second index.
///
/// Turn identity inside the thread is deterministic (`"<loopId>_pm<round>"`,
/// `"<loopId>_dev<round>"`, `"<loopId>_escalate<round>"`, `"<loopId>_stopped"`) so
/// `sync(state:now:)` is idempotent and safe to call after EVERY `LoopState` mutation
/// (it is — `LoopCoordinator.persist` calls it unconditionally): each call only appends
...
///
/// PM/dev text is never carried on `LoopState`/`RelayRound` (run-truth lives in
...
/// root the coordinator dispatched through, exactly like `LoopCoordinator.devReportText`
/// does for the next round's PM prompt.
public struct LoopThreadProjector: Sendable {
    ...
    // MARK: - Entry points (called by LoopCoordinator)
    ...
    /// `LoopCoordinator.Config.projectId` is in scope; `LoopState` itself doesn't carry
    /// it (PM_Relay.md: per-invocation, not persisted). Best-effort: a `ThreadStore`
    /// failure here never aborts the relay — the relay's own durability is `LoopState`.
    public func started(state: LoopState, projectId: String?) {
    ...
    /// EVERY `LoopState` persist (`LoopCoordinator.persist`) — the single choke point
    ...
    public func sync(state: LoopState, now: Date) {
    ...
    private func syncPMTurn(round: RelayRound, state: LoopState, thread: inout WorkThread, now: Date) {
        ...
        if let pmRunId = round.pmRunId, existing.runId != pmRunId,
           let text = runStore.load(runId: pmRunId)?.answers.first?.output {
            ...
            // `LoopCoordinator.finishRound` persists TWICE for this outcome (once with
    ...
    /// there is no PM model dispatch to wait on — `LoopCoordinator.runExternalRound`
    ...
    private func syncDevTurn(round: RelayRound, state: LoopState, thread: inout WorkThread, now: Date) {
    ...
    /// `LoopCoordinator.resume` flips it back to `.running`) — mirroring
    /// `AgentChatCoordinator.completeManualPaste`'s "resolve the open note" shape.
    private func syncEscalation(round: RelayRound, state: LoopState, thread: inout WorkThread, now: Date) {
        ...
        if var existing = thread.turn(id: turnId) {
            ...
            // Still the CURRENT escalation (not yet resumed): `LoopCoordinator.finishRound`
    ...
    /// not tied to any one round (the ceiling check in `LoopCoordinator.loop` can fire
    /// before a round even starts), so this is keyed once per relay rather than per round.
    private func syncStopped(state: LoopState, thread: inout WorkThread, now: Date) {
