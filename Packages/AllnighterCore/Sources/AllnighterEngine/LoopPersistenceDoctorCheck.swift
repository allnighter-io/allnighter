...
public enum LoopPersistenceDoctorCheck {
    ...
    public static func scan(loopsRoot: URL) -> [Finding] {
        guard let entries = try? FileManager.default.contentsOfDirectory(
            at: loopsRoot, includingPropertiesForKeys: nil
        ) else { return [] }

        let store = LoopStateStore(rootDirectory: loopsRoot)
        var findings: [Finding] = []
        for directory in entries where directory.lastPathComponent.hasPrefix("relay_") {
            let loopId = directory.lastPathComponent
            let path = directory.appendingPathComponent("relay.json").path
            switch store.loadResult(id: loopId) {
            case .success:
                continue
        ...
        return findings.sorted { $0.loopId < $1.loopId }
    }

    public static func doctorCheck(loopsRoot: URL) -> DoctorResult.Check {
        let findings = scan(loopsRoot: loopsRoot)
        ...
        let sample = findings.prefix(3).map(\.loopId).joined(separator: ", ")
