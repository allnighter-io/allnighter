...
/// File-backed registry of panel presets under `Config/TeamPresets/`. The
...
public struct TeamPresetStore: Sendable {
    public let rootDirectory: URL

    public init(rootDirectory: URL? = nil) {
        self.rootDirectory = rootDirectory
            ?? AllnighterPaths.config.appendingPathComponent("TeamPresets", isDirectory: true)
    }

    /// User-saved presets, id-sorted for stable display.
    public func load() -> [TeamPreset] {
        ...
        return entries
            ...
            .compactMap { try? CoreJSON.decode(TeamPreset.self, from: Data(contentsOf: $0)) }
    }

    @discardableResult
    public func save(_ preset: TeamPreset) throws -> URL {
