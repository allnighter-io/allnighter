...
///
/// This is a SAFETY rule before it is a routing convenience, which is why it
/// lives in code and not only in a skill prompt: running a media downloader
/// (`vvx` → yt-dlp) against an X/Twitter URL risks the user's own X account
/// under X's terms. X links are therefore read by an X-capable model (Grok) or
...
public enum SignalSourceRouter {
    ...
    public static func route(forURL raw: String) -> SignalSourceRoute {
        ...
        return xHosts.contains(host) ? .xModelOnly : .videoTool
    }
