...
public enum TeamPreflight {
    public struct WorkerRef: Codable, Sendable, Equatable {
        ...
        public init(workerId: String, skillId: String?, skillName: String?, modelId: String, purpose: String) {
            self.workerId = workerId; self.skillId = skillId; self.skillName = skillName
            self.modelId = modelId; self.purpose = purpose
        }
    }
    ...
    public static func preflight(
        ...
        case .success(let req):
            ...
            let workers = resolved.allWorkers.map {
                WorkerRef(workerId: $0.id, skillId: $0.skillId, skillName: $0.skillName,
                          modelId: $0.modelId, purpose: ($0.purpose ?? .answer).rawValue)
            }
            ...
            return Result(
                canStart: resolved.isRunnable, lane: req.lane.rawValue, teamPresetId: req.team.id,
                ...
                selfFusion: fusion, warnings: resolved.warnings, blockedReason: resolved.blockReason,
                nextAction: resolved.isRunnable
                    ? AgentNextAction(kind: "startTeamRun", tool: "team_start")
                    : AgentNextAction(kind: "runDoctor", tool: "doctor"))
        }
    }
}
