...
public enum AIReadinessShape {
    public static func detect(
        ...
        let fileExists = fileExists ?? { name in FileManager.default.fileExists(atPath: root.appendingPathComponent(name).path) }
        let fileContents = fileContents ?? { name in try String(contentsOf: root.appendingPathComponent(name), encoding: .utf8) }
        let entryNames = entryNames ?? { try FileManager.default.contentsOfDirectory(atPath: root.path) }
        var hasSwiftApp = false
        var hasTS = false
        var hasCLI = false

        if let entries = try? entryNames() {
        ...
        if fileExists("Package.swift") {
            ...
            if let contents = try? fileContents("Package.swift"),
               (contents.contains(".executableTarget(") || contents.contains("executableTarget")) {
                hasCLI = true
            }
        }
        ...
        if fileExists("package.json") {
            ...
            if hasTSConfig {
                hasTS = true
            } else if let contents = try? fileContents("package.json"), contents.contains("\"typescript\"") {
                hasTS = true
            }
            if !hasTSConfig, let contents = try? fileContents("package.json"), contents.contains("\"bin\"") {
                hasCLI = true
            }
        }
        ...
        if fileExists("Cargo.toml"), let contents = try? fileContents("Cargo.toml"), contents.contains("[[bin]]") {
            hasCLI = true
        }

        if fileExists("main.go") || fileExists("main.swift") {
            hasCLI = true
        }

        if hasSwiftApp && hasTS {
            return .unclear
        }
        if hasSwiftApp {
            return .swiftAppleApp
        }
        if hasTS {
            return .typeScriptAppOrMonorepo
        }
        if hasCLI {
            return .cliTool
        }
    ...
    public static func brief(for fingerprint: AIReadinessFingerprint) -> String {
        ...
        case .unclear:
            ...
            The repo shape could not be determined with confidence. Ask universal questions only — do not assume a specific technology stack.
            """
        }
    }
}
