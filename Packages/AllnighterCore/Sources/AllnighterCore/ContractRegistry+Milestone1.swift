...
public extension ContractRegistry {
    ...
    static let m1Errors: [ErrorSpec] = [
        ...
        ErrorSpec("SERVE_INSTALL_FAILED", ruleId: "serve.install.failed", agentAction: "Read the message for the failing step, then run `alln serve repair`. If the canonical binary is missing, run `alln install-cli` first.", requiresManual: true, retryable: true, explain: "A serve install/uninstall step failed — bootout before a byte change, plist write, or registration. The transaction reports the failing step rather than continuing; §4.3 restores the prior binary, symlink and plist on failure."),
        ErrorSpec("SERVE_FOREIGN_HOME", ruleId: "serve.foreign_home", agentAction: "Run the serve lifecycle command with the real user HOME shown in the error. A per-user launchd label cannot safely be managed from a foreign HOME.", requiresManual: true, retryable: false, explain: "Serve lifecycle mutations are refused when effective HOME differs from the account home because launchd scopes com.allnighter.resident-coordinator to one user, not one HOME directory."),
    ]
