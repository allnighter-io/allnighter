...
public extension ContractRegistry {
    ...
    static let m1Errors: [ErrorSpec] = [
        ...
        ErrorSpec("COORDINATOR_UNAVAILABLE", ruleId: "coordinator.unavailable", agentAction: "Run `alln serve install --json` once from the normal macOS Terminal app, then retry; a sandboxed agent host cannot install the background coordinator and foreground spawning is not a fallback.", requiresManual: true, retryable: false, explain: "The resident coordinator is not reachable. Allnighter never falls back to a caller-owned vendor process because that would reintroduce sandbox and privacy-prompt failures."),
        ...
        ErrorSpec("PANEL_SEAT_NOT_ISOLATED", ruleId: "panel.seat.not_isolated", agentAction: "Retry the panel round. If it persists, free disk space and confirm the project root is readable — clone isolation failed to materialize for a non-RO-enforcing seat.", requiresManual: false, retryable: true, explain: "A panel seat needed ephemeral clone isolation (driver has no confirmed RO mode) but the clone could not be materialised (copy failed, source unreadable, or disk full). Seats are never refused for lacking a RO mode — this code is only for clone materialization failure."),
