...
public extension ContractRegistry {
    ...
    static let m1MCPTools: [MCPToolSpec] = [
        ...
        MCPToolSpec("teams_save", command: "teams edit", summary: "Save a team definition in place (full replacement). Editing a built-in writes the user's version at the same id; the shipped version stays available via teams_restore — no duplicate.",
                    params: [.init("teamId", required: true, summary: "Team id."),
                             .init("definition", required: true, summary: "TeamPreset JSON object.")],
                    errors: ["TEAM_INVALID", "CATALOG_ID_INVALID", "SKILL_LANE_MISMATCH", "CLI_USAGE_ERROR", "INTERNAL_ERROR"], idempotency: .idempotent),
        ...
        MCPToolSpec("teams_delete", command: "teams delete", summary: "Delete a custom team. For a built-in, removes the user's edits and restores the shipped version (fails if it was never edited).",
                    params: [.init("teamId", required: true, summary: "Team id.")],
                    errors: ["TEAM_NOT_FOUND", "TEAM_BUILTIN_IMMUTABLE", "TEAM_DEFAULT_INVALID", "CLI_USAGE_ERROR", "INTERNAL_ERROR"], idempotency: .idempotent),
        MCPToolSpec("teams_restore", command: "teams restore", summary: "Restore a built-in team to its shipped version by removing the user's edits. Idempotent.",
                    params: [.init("teamId", required: true, summary: "Team id.")],
                    errors: ["TEAM_RESTORE_UNSUPPORTED", "TEAM_NOT_FOUND", "CLI_USAGE_ERROR", "INTERNAL_ERROR"], idempotency: .idempotent),
        ...
        MCPToolSpec("team_preflight", command: "team preflight", summary: "Validate lane/team/effort against the ready bench WITHOUT running or spending quota; shows resolved/blocked workers and self-fusion.",
                    ...
                    errors: ["DEFAULT_TEAM_INVALID", "TEAM_NOT_FOUND", "MODEL_UNAVAILABLE", "TEAM_GOVERNOR_BUSY", "TEAM_GOVERNOR_UNAVAILABLE", "CLI_USAGE_ERROR"], idempotency: .idempotent),
        MCPToolSpec("team_start", command: "team start", summary: "Start an async team run; returns run id after preflight and journal write. Pass dryRun:true to validate without spending quota.",
                    params: [.init("prompt", required: true, summary: "The prompt to run."),
                             .init("dryRun", type: "boolean", summary: "When true, run preflight only (no run, no quota)."),
                    ...
                    errors: ["DEFAULT_TEAM_INVALID", "TEAM_NOT_FOUND", "MODEL_UNAVAILABLE", "NESTED_TEAM_BLOCKED", "TEAM_GOVERNOR_BUSY", "TEAM_GOVERNOR_UNAVAILABLE", "IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_PAYLOAD", "COORDINATOR_UNAVAILABLE", "CLI_USAGE_ERROR"],
                    idempotency: .keyed),
        ...
        MCPToolSpec("pair_slice", command: "pair slice", summary: "Dispatch one bounded work slice: gate → executor → repo-declared check. Planner seat is recorded; executor runs the packet. Pass inline `packet` JSON or a `packetPath` on disk.",
                    params: [.init("project", required: true, summary: "Project id, name, or repo path."),
                             .init("packetPath", summary: "Path to a WorkSlicePacket .json or markdown with a work-slice-packet fence."),
                             .init("packet", type: "object", summary: "Inline WorkSlicePacket JSON (alternative to packetPath)."),
                             .init("executor", summary: "Mutating executor team id (default default_chat)."),
                             .init("plannerWorker", summary: "Planner model id recorded on the slice envelope (default Composer 2.5)."),
                             .init("executorWorker", summary: "Executor model id (default Gemini 3.5 Flash).")],
                    outputSchema: .pairSliceJSON,
                    errors: ["PROJECT_NOT_FOUND", "PAIR_SLICE_PACKET_MISSING", "PAIR_SLICE_UNSAFE", "PAIR_SLICE_EXECUTOR_INVALID", "PAIR_SERVE_UNAVAILABLE", "RUN_WRITE_LOCK_BUSY", "WORKER_NOT_READY", "CLI_USAGE_ERROR"],
                    idempotency: .notIdempotent),
        MCPToolSpec("pair_run", command: "pair run", summary: "Run a durable slice queue unattended: executor attempts (default 3) with escalating nudges, then planner takeover once per packet, then advance.",
                    params: [.init("project", required: true, summary: "Project id, name, or repo path."),
                             .init("queueDir", required: true, summary: "Directory of slice .json packets (and optional queue.json state)."),
                             .init("until", summary: "Hard stop clock HH:MM (local)."),
                             .init("maxRetries", type: "integer", summary: "Legacy stall nudge cap (optional)."),
                             .init("executorAttemptsBeforePlanner", type: "integer", summary: "GLM attempts per packet before planner takeover (default 3)."),
                             .init("executor", summary: "Mutating executor team id (default default_chat)."),
                             .init("plannerWorker", summary: "Planner model id — recorded on slices and invoked on takeover (default Composer 2.5)."),
                             .init("executorWorker", summary: "Executor model id (default Gemini 3.5 Flash).")],
                    outputSchema: .pairQueueJSON,
                    errors: ["PROJECT_NOT_FOUND", "PAIR_SLICE_PACKET_MISSING", "PAIR_SLICE_UNSAFE", "PAIR_SLICE_EXECUTOR_INVALID", "PAIR_SERVE_UNAVAILABLE", "RUN_WRITE_LOCK_BUSY", "WORKER_NOT_READY", "CLI_USAGE_ERROR"],
                    idempotency: .notIdempotent),
        MCPToolSpec("pair_status", command: "pair status", summary: "Read slice queue state: per-entry status, child run id, elapsed time since child run started, and stall guidance.",
        ...
        MCPToolSpec("team_ask", command: "team", summary: "Run a lane team on a prompt; returns a synthesized result + structured run.",
                    ...
                    outputSchema: .teamRunJSON,
                    errors: ["DEFAULT_TEAM_INVALID", "TEAM_NOT_FOUND", "MODEL_UNAVAILABLE", "TEAM_RUN_TIMEOUT", "TEAM_RUN_FAILED", "PLAN_WRITER_FAILED", "WORKER_FAILED", "NESTED_TEAM_BLOCKED", "TEAM_GOVERNOR_BUSY", "TEAM_GOVERNOR_UNAVAILABLE", "CLI_USAGE_ERROR"],
                    idempotency: .notIdempotent),
        ...
        MCPToolSpec("thread_send", command: "thread send", summary: "Send a message and/or images to a work thread worker.",
                    ...
                    errors: ["THREAD_NOT_FOUND", "THREAD_SEND_IDEMPOTENCY_CONFLICT", "WORKER_FAILED", "THREAD_SEND_FAILED", "ATTACHMENT_TOO_MANY", "ATTACHMENT_TOO_LARGE", "ATTACHMENT_UNSUPPORTED_TYPE", "ATTACHMENT_DECODE_FAILED", "FILE_REFERENCE_OUTSIDE_PROJECT", "FILE_REFERENCE_NOT_FOUND", "FILE_REFERENCE_TOO_LARGE", "FILE_REFERENCE_TOO_MANY", "CLI_USAGE_ERROR"],
                    idempotency: .keyed),
