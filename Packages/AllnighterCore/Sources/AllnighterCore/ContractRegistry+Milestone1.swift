...
public extension ContractRegistry {
    ...
    static let m1Errors: [ErrorSpec] = [
        ...
        ErrorSpec("RELAY_PM_READONLY_UNSUPPORTED", ruleId: "relay.pm_readonly.unsupported", agentAction: "Pick a pm-worker on a driver with a confirmed mechanical read-only mode (currently claude_code or codex — see RelayReadOnlyEnforcer.supportedDriverIds), or drop --pm-read-only / pmReadOnly.", requiresManual: true, retryable: false, explain: "--pm-read-only (a mechanically non-mutating PM seat) was requested, but the pm-worker's driver has no confirmed read-only/sandbox mode. Fails closed at start — never a silent prompt-only \"don't write\" fallback.", exitClass: .usage),
