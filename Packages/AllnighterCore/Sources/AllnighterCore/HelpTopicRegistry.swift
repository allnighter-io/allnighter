    ...
    public static let topics: [HelpTopic] = [
        ...
        HelpTopic(
            ...
            sections: [
                ...
                .init("local-dev", "Local execution seat", "`alln loop start \"…\" --pm <frontier-id> --dev <local-ollama-id>` is the packet headline: a frontier seat plans, a local Ollama-backed seat executes, under the same per-root write lock. Pin the local seat with `--dev` only — `--pm` refuses a local occupant (`LOOP_LOCAL_SEAT_CANNOT_LEAD`). Failure is the common case. Allnighter stamps `roundLog.executionOutcome` from worker status, repo delta, and proofs — never from the seat's report. A local seat that claims \"ready for review\" while proofs fail (or the tree did not move) is `failed` for the lead to act on, not a silent or false success."),
            ...
            errorRefs: [
                ...
                "RELAY_ALREADY_ACTIVE", "LOOP_LOCAL_SEAT_CANNOT_LEAD",
            ],
            needsLiveCheck: true),
