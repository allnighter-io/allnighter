...
| Agent front door: `install-cli`, `bootstrap`, first-contact counsel (`models`, `team hello`, `doctor`) | Front door V1 Complete — archived `docs/archive/phases/Agent_Front_Door.md` (gate 1) + archived `docs/archive/phases/Agent_Onboarding.md` (gate 2) + archived `docs/archive/phases/Agent_Intent_Router.md` (gate 3, code SSOT `AgentIntentRouter.swift`) |
| Stale MCP / invented flags in help, empty `help search`, dead verbs in living docs, `version` freshness | `docs/phases/CLI_Agent_Surface_Fidelity.md` + `docs/workflows/SSOT_Founder_Input_Workflow.md` §Agent-facing help |
