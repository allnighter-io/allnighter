...
| Multi-seat run looks dispatched but a seat never worked; PM agent needs to pick another model | Open packet: `docs/phases/Crew_Understaffed_Signal.md` — top-level `crew_understaffed` warning on `alln show`; no mid-run reseat in v1. Code SSOT TBD `TeamRunJSONMapper` |
...
| OpenCode local / Ollama seats (Studio-class local models via OpenCode) | Brainstorm only: `docs/phases/OpenCode_Local_Ollama_Seats.md` — same `opencode` driver as Go; local provenance; ignore Ollama Cloud; no slice authorized |
...
| OpenCode long runs / concurrent `stream_drop` / subagent (`task`) hang | Open packet: `docs/phases/OpenCode_Long_Run_Continuity.md` — post-S122 dogfood; code SSOT AgentOS `OpenCodeServeClient`/`OpenCodeSSEParser`, Allnighter `RunService` observation + outcome gates |
