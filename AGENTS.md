...
| Pure model name / family latest / generation pin | Vocabulary `Product_Vocabulary.md` §pure name; AgentOS `FamilyLatest` + `catalog.json`; run journal `ModelPinFact` |
...
| Capacity influencing seat **selection** / automated quota routing | **KILLED 2026-08-12 (feature creep)** — archived `Capacity_Warm_Bench.md`. Covers automated routing ONLY. `alln capacity` is DAILY-USE: the founder reads the strip and routes by hand. Never degrade acquisition for any source; "no code consumer" ≠ "no consumer" |
