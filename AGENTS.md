...
| Predictive capacity: quota dashboards, credential-backed quota readers, capacity influencing seat **selection** | **KILLED 2026-08-12 (founder: feature creep).** Capacity changes no decision — `TeamResolver` has no capacity references and `alln menu` emits no `capacity`. Runtime park/substitute is reactive and vendor-stated; that is enough. Do not build credential storage for a dashboard. Archived: `Capacity_Warm_Bench.md`, `OpenCode_Go_Capacity.md`, `Bailian_Token_Plan_Capacity.md` |
