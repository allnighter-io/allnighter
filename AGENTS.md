...
| OpenCode Go plan capacity (browser `/go` scrape, encrypted credentials) | `docs/phases/OpenCode_Go_Capacity.md` — **CLI beta SHIPPED, metering live**; code SSOT `OpenCodeGoCapacity*` |
| Capacity influencing seat **selection** / automated quota routing | **KILLED 2026-08-12 (founder: feature creep)** — the resident trust gate and 8h soak certified numbers no code acts on. Archived `Capacity_Warm_Bench.md`. **This kill covers automated routing ONLY.** `alln capacity` is a DAILY-USE surface: the founder reads the strip and routes work by hand. Reading capacity is load-bearing product — do not delete or degrade acquisition for any source, and never assume "no code consumer" means "no consumer" |
...
| Wrong/invented capacity verdict; cross-vendor limit detection; a seat parked that was healthy | Code SSOT: AgentOS `CapacityClassifier` (every matcher scoped by `sourceId`), `VendorBackoffPolicy.shouldPark`, `CapacityPaintGate` (stale ⇒ unknown), `SeatReseat`. Capacity is a sensor: it never durably parks (that is user intent via `alln drivers park`) and never gates seat selection. Closed record: archived `Vendor_Signal_Isolation.md` |
