...
| Run started / stuck / kill / retry / missing stream | `alln show <id> --json` / `--stream`. Git landing is not completion (`StuckRunDisclosure`). Code: `RunService`, `TeamRunJSONMapper`, `RemoteRunEventJournal`, `KillSettlement`, `RunClockEnforcer`, `IdempotencyStore`, `ProcessOwnership`. |
...
| Ship / bump / push / release / publish (CLI or DMG); "run / install the **latest**" | `docs/operations/Public_Release.md` § **Agent ship intent** — run `scripts/ship-cli.sh` / `build-dmg.sh` yourself; founder wording = approval. "Latest" = notarized CLI **and** app, both installed and verified by identity (§ item 6) — never an ad-hoc `rebuild_cli.sh` binary. `--upload` is not done until `faucet-smoke.sh` (live `curl \| sh` + bare PATH `menu --json`). Not `docs/qa/` gates unless explicitly ordered. |
