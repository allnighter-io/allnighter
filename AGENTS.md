...
| Ship / bump / push / release / publish (CLI or DMG) | `docs/operations/Public_Release.md` § **Agent ship intent** — run `scripts/ship-cli.sh` / `build-dmg.sh` yourself; founder wording = approval. `--upload` is not done until `faucet-smoke.sh` (live `curl \| sh` + bare PATH `menu --json`). Not `docs/qa/` gates unless explicitly ordered. |
