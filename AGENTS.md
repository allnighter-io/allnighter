...
| Multi-seat / `--seat` on a spawn-gated CLI (cursor_agent, opencode, agy); same-CLI crew on large teams | **Closed 2026-08-08** — serialize, never refuse. The gate wait is bounded by the seat's own `invoke.timeoutSeconds`, not a fixed 300s, so a waiter is not dropped while a sibling holds the slot; `--dry-run` emits `seat_driver_serialized:` naming the driver and its seats. Code SSOT: AgentOS `GatedWorkerRunner.invoke`; Allnighter `RunInvocationResolver.spawnSerializationWarnings` |
...
| Serve-hosted capacity refresh residuals (double-probe, orphan PTY, partial-fail stale seats, sync refresh starvation) | Open packet: `docs/phases/Capacity_Serve_Refresh_Polish.md` (CRS-S01…S05) — code SSOT `CapacityRefreshScheduler`, `ServeDaemon`, `CapacityFetch` / `CapacityHistoryStore`; PF-S03 shipped |
...
| OpenCode `:4096` leftover serve between runs | **Closed 2026-08-08** — a healthy listener is attached, not refused; identity is port + health, never spawn provenance. `portOwnedByForeignProcess` no longer exists. Never SIGTERM a healthy listener to clear the port. Code SSOT: AgentOS `OpenCodeServeCoordinator.ensureRunning`; help topic `opencode_headless_completion` |
| OpenCode local / Ollama seats; frontier seat delegating to local labor; sweep over many targets; anyone proposing a local-vs-cloud benchmark or tok/s figure | Open packet **1 of 3**: `docs/phases/OpenCode_Local_Ollama_Seats.md` — headline is *a frontier seat plans, local seats execute* (§2.4); agnostic bodies; `ollama_local` readiness not capacity; ignore Ollama Cloud; benchmarking is a non-goal; code unauthorized until ready |
| Context Firewall / egress policy / egress ledger / root-less dispatch ("keep the frontier model away from my source"); privacy or regulated buyers | Open packet **2 of 3**: `docs/phases/Context_Firewall.md` — the claim is **auditable egress, never sanitisation** (§4.1, blocking copy review); refusal is user intent not a readiness veto; root-less dispatch (§6) is undesigned and blocks code; regulated tier is bottom-up optionality, no enterprise motion |
| Second Mac, Studio in the office, LAN bench, remote `OLLAMA_HOST`, multi-host execution | Open packet **3 of 3**: `docs/phases/Second_Mac_Bench.md` — a fence, not a plan. The shelved LAN architecture stays shelved; refuse cross-host mutators and network-held write locks |
...
| OpenCode empty `incomplete_no_final_message` after idle; parser race; stall mid-`task`; seat timeout 2× | **Closed 2026-08-09** — OCH-S01…S04 in AgentOS `65da768` (actor parser last-only, harden reconcile, busy/idle-defer + clock, shared seat budget). Archived: `docs/archive/phases/OpenCode_Turn_Capture_Hardening.md`. Help `opencode_headless_completion`. |
