...
| Context Firewall / egress policy ("keep the frontier model away from my source"); frontier seat delegating to local labor; sweep over many targets; anyone proposing a local-vs-cloud benchmark | Same packet §2.4 (delegation asymmetry), §2.6 (workflow catalog), §7.8 (egress law) — benchmarking and any claim of *sanitisation* are non-goals; root-less dispatch (§13.5) is undesigned and blocks firewall code |
