...
| Multi-seat / `--seat` drops a seat on Cursor/OpenCode spawn gate; same-CLI crew on large teams | Open packet: `docs/phases/Crew_Understaffed_Signal.md` — serialize don't drop; dry-run names serial drivers; do not refuse same-CLI. Code SSOT `DriverConcurrencyGate`, `TeamExplicitSeats` |
