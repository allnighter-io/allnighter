...
| One-off team seating / custom `*_` picker sprawl / “staff these models once” | Open packet: `docs/phases/Ephemeral_Teams.md` — runtime `--seat` on `alln run` (Option C locked); do not `teams duplicate` for one-offs |
