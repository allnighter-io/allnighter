...
| Plan-time quota routing, capacity in menu/bootstrap, loop session-cap wake | Open packet: `docs/phases/Quota_Aware_Bench_Continuity.md` — code SSOT `CapacityDisplayAcquisition`, `MenuCatalog`, `RelayCoordinator.dispatchDevTurn`, `VendorBackoffReconciler`, `VendorSubstitutionPolicy` |
...
| Test / CI / `check.sh` / agent proof commands / test pile-ups | `docs/phases/Test_Infrastructure_Upgrade.md` (FINAL rev 2, TIU-S00–S03 all core; no separate runbook — rules land in Execution Playbook) |
