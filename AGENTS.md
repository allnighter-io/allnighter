...
| `pair pilot status` fresh silenceAge while `alln ps` says no stream for Ns; hung child under worker (e.g. wrangler tail); pgid heartbeat lie | open `docs/phases/Pilot_Status_Liveness_Lie_Hotfix.md` — PRIMARY must be `run.lastActivityAt` only; code SSOT `PilotCLI.resolveLastProgressAt` |
